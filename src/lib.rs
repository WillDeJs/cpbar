...
#[doc(hidden)]
pub trait ProgressBarDisplay where Self: Sized {
...
impl ProgressBarDisplay for Bounded{
    fn display<Iter>(&self, progress: &ProgressBar<Iter, Self>) {
        println!("{}{}{}{}{}", CLEAR, self.delims.0, "▓".repeat(progress.index), 
                "░".repeat(self.bound - progress.index), self.delims.1); 
    }
}

impl ProgressBarDisplay for Unbounded {
    fn display<Iter>(&self, progress: &ProgressBar<Iter, Self>) {
        println!("{}{}", CLEAR, "▓".repeat(progress.index));
    }
} 

impl<Iter> ProgressBar <Iter, Unbounded> where Iter: Iterator{
    ...
    pub fn new(iter: Iter) -> Self {
        println!();
        Self {iter, index: 0, bound: Unbounded}
    }
}

impl<Iter, Bound> Iterator for ProgressBar<Iter, Bound> where Iter: Iterator, Bound: ProgressBarDisplay {
    type Item =  Iter::Item;
...
impl<Iter> ProgressBar<Iter, Unbounded> where Iter: ExactSizeIterator {
    ...
    pub fn with_bounds(self) -> ProgressBar<Iter,Bounded>{
        ...
        ProgressBar{iter:self.iter, bound, index: self.index}
    }   
}

impl<Iter> ProgressBar<Iter,Bounded> where Iter: ExactSizeIterator {
